...
Current task list: @docs/todo.txt
Feedback inbox: @FEEDBACK.md

The product spec is `docs/vision.md`. Do not read all of it — every task
...
**Only the lines below the `---` in `FEEDBACK.md` are feedback** — everything above it is the
...
- Then append the **raw, unedited lines** to `docs/feedback-archive.md` under a `## <today>`
  heading, and reset `FEEDBACK.md` to just its header and the `---`.
...
Then do the same for `docs/agent-findings.md`, the agent's own queue, tagging those `+found
...
prototype actually shipped — read what went wrong in `docs/goals/initial-prototype/feedback.md`
...
- When you are working out *what is happening* rather than building, read
  `docs/way-of-working/diagnosis.md` first. Four rules, each one paid for: measure the round trip,
...
- Prefer CSS over JavaScript for anything layout, type, or motion. That is the entire reason this
  app is not in Rust any more — see `docs/path-forward.md`.
...
These come from `docs/agent-development-time-postmortem.md`, where ignoring them cost 18 hours:

- **No subagent fanout.** Zero, or one if a search is genuinely wide. Not five.
- **New problems you notice go in `docs/agent-findings.md`, not into this session.** The only
  exceptions are data loss, a crash, or a security hole. Scope that grows mid-session never
  converges. **Never write to `FEEDBACK.md`** — that inbox is the human's, and the value of
  `docs/feedback-archive.md` as evidence depends on it holding only their words.
- **Never run this under a loop with no terminal condition.** `/loop 60s /session until you reach
  the next checkpoint` is the intended way to work — that condition is a line in `docs/todo.txt`

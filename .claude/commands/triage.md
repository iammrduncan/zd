---
description: Turn raw docs/_objectives/FEEDBACK.md notes into todo.txt tasks, without doing any of the work
...
2. Write it as a todo.txt task matching the surrounding format:
   ...
   - Add `+p<N>` for the phase, `@ctx` for the area, `vis:N.N` for the spec section, and `ref:F##`
     if it matches a first-prototype finding in `docs/_objectives/goals/initial-prototype/feedback.md`.
...
5. If it contradicts `docs/_objectives/vision.md`, say so rather than silently
   rewriting the vision. That is a decision for the user.

Then:

- Append the **raw, unedited lines** to `docs/_objectives/feedback-archive.md` under a `## <today>` heading
  (`date +%F`). Their words, not your summary — the raw complaint is the evidence.
- Reset `docs/_objectives/FEEDBACK.md` to its header and the `---`.
- Then triage `docs/_objectives/agent-findings.md` the same way, tagging those `+found found:<today>`. It is
  the agent's own queue; human feedback is triaged first and outranks it. Never write findings
  into `docs/_objectives/FEEDBACK.md` — keeping the archive purely human is what makes it evidence.

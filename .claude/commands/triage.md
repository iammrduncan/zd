---
description: Turn raw FEEDBACK.md notes into todo.txt tasks, without doing any of the work
...
2. Write it as a todo.txt task matching the surrounding format:
   ...
   - Add `+p<N>` for the phase, `@ctx` for the area, `vis:N.N` for the spec section, and `ref:F##`
     if it matches a first-prototype finding in `docs/goals/initial-prototype/feedback.md`.
...
- Append the **raw, unedited lines** to `docs/feedback-archive.md` under a `## <today>` heading
  (`date +%F`). Their words, not your summary — the raw complaint is the evidence.
- Reset `FEEDBACK.md` to its header and the `---`.
- Then triage `docs/agent-findings.md` the same way, tagging those `+found found:<today>`. It is
  the agent's own queue; human feedback is triaged first and outranks it. Never write findings
  into `FEEDBACK.md` — keeping the archive purely human is what makes it evidence.

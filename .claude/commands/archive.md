...
1. **Count first.** `grep -c '^x ' docs/todo.txt` and `grep -vc '^x ' docs/todo.txt`. State both
...
3. Append them **verbatim and in file order** to `docs/todo-archive.txt`, under a `## <today>`
   heading (`date +%F`). Create the file with this header if it does not exist:

   ```
   # zd md — completed tasks, moved out of docs/todo.txt by /archive
   # Same format as the task list. Newest block last. Nothing here is edited or deleted.
   ```

4. Remove exactly those lines from `docs/todo.txt`. Everything else stays untouched and in its
...
- Open-task count is identical to step 1. **This is the check that matters**: if it moved, you
  deleted live work — `git checkout docs/todo.txt` and stop.
- Lines added to `docs/todo-archive.txt` equal lines removed from `docs/todo.txt`.
- `grep -c '^x ' docs/todo.txt` is 0, or exactly the finished lines a filter excluded.

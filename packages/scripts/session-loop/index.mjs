...
const MEMORY_PATH = resolve("docs/_internal/objectives/session-memory.log");
const PROMPT =
...
Run one $zd-session at a time until docs/_internal/objectives/FEEDBACK.md is empty and the next open
CHECKPOINT in docs/_internal/objectives/todo.txt is reached.
...
function readPlan() {
  ...
  if (checkpointIndex === -1)
    fail("No open checkpoint remains in docs/_internal/objectives/todo.txt; refusing to loop");
...
function readFeedback() {
  if (!existsSync(FEEDBACK_PATH))
    fail(
      "docs/_internal/objectives/FEEDBACK.md does not exist; run this command from the repository root",
    );

  const contents = readFileSync(FEEDBACK_PATH, "utf8");
  const lines = contents.split("\n");
  const dividerIndex = lines.findIndex((line) => line.trim() === "---");
  if (dividerIndex === -1) fail("docs/_internal/objectives/FEEDBACK.md has no --- inbox divider");
...
function printDryRun(state, waitLabel, waitMs, startCommit) {
  ...
  if (feedback.entries.length > 0) {
    ...
    console.log(
      "\n1. Triage docs/_internal/objectives/FEEDBACK.md, then take the first eligible task in the live band",
    );
  ...
  while (true) {
    ...
    const task =
      state.feedback.entries.length > 0
        ? `Triage ${state.feedback.entries.length} pending docs/_internal/objectives/FEEDBACK.md entries, then take the first eligible task`
        : state.plan.tasks[0];
    ...
    if (!todoChanged && !feedbackChanged) {
      ...
      fail(
        "Neither docs/_internal/objectives/todo.txt nor docs/_internal/objectives/FEEDBACK.md changed; stopping to avoid repeating the same feedback session",
      );
    }

...
const TODO_PATH = resolve("docs/planning/objectives/todo.txt");
const FEEDBACK_PATH = resolve("docs/planning/objectives/FEEDBACK.md");
const MEMORY_PATH = resolve("docs/planning/objectives/session-memory.log");
const PROMPT =
...
Run one $zd-session at a time until docs/planning/objectives/FEEDBACK.md is empty and the next open
CHECKPOINT in docs/planning/objectives/todo.txt is reached.
...
function readPlan() {
  if (!existsSync(TODO_PATH))
    fail(
      "docs/planning/objectives/todo.txt does not exist; run this command from the repository root",
    );
  ...
  if (checkpointIndex === -1)
    fail("No open checkpoint remains in docs/planning/objectives/todo.txt; refusing to loop");
...
function readFeedback() {
  if (!existsSync(FEEDBACK_PATH))
    fail(
      "docs/planning/objectives/FEEDBACK.md does not exist; run this command from the repository root",
    );

  const contents = readFileSync(FEEDBACK_PATH, "utf8");
  const lines = contents.split("\n");
  const dividerIndex = lines.findIndex((line) => line.trim() === "---");
  if (dividerIndex === -1) fail("docs/planning/objectives/FEEDBACK.md has no --- inbox divider");
...
function printDryRun(state, waitLabel, waitMs, startCommit) {
  ...
  if (feedback.entries.length > 0) {
    ...
    console.log(
      "\n1. Triage docs/planning/objectives/FEEDBACK.md, then take the first eligible task in the live band",
    );
  ...
  while (true) {
    ...
    const task =
      state.feedback.entries.length > 0
        ? `Triage ${state.feedback.entries.length} pending docs/planning/objectives/FEEDBACK.md entries, then take the first eligible task`
        : state.plan.tasks[0];
    ...
    if (!todoChanged && !feedbackChanged) {
      ...
      fail(
        "Neither docs/planning/objectives/todo.txt nor docs/planning/objectives/FEEDBACK.md changed; stopping to avoid repeating the same feedback session",
      );
    }

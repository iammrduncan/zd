...
function makeFixture(todo: string, feedback = "# Feedback\n\n---\n"): string {
  ...
  writeFileSync(join(root, "docs", "todo.txt"), todo);
  writeFileSync(join(root, "FEEDBACK.md"), feedback);
  spawnSync("git", ["init", "-q"], { cwd: root });
  spawnSync("git", ["add", "docs/todo.txt", "FEEDBACK.md"], { cwd: root });
  spawnSync(
    "git",
    [
      "-c",
...
`);
    ...
    expect(existsSync(join(root, "docs", "session-memory.log"))).toBe(false);
  });
...
`);
  ...
  it("refuses to run a plan with no future checkpoint", () => {
    ...
    expect(result.stderr).toContain(
      "No open checkpoint remains in docs/todo.txt; refusing to loop",
    );
  });
...
describe("the Codex session loop", () => {
  it("pauses at @DECIDE, accepts an answer in the TUI, and passes it to Codex", async () => {
    ...
    writeFileSync(
      join(root, "docs", "session-memory.log"),
...
const todoPath = join(root, "docs", "todo.txt");
...
`,
    ...
    expect(readFileSync(join(root, "docs", "todo.txt"), "utf8")).toContain(`x 2026-08-03 ${task}`);
  });
...
`,
    ...
    const memory = readFileSync(join(root, "docs", "session-memory.log"), "utf8");
    expect(memory).toContain("# Testing summary\n\n- Check the completed task.");
    expect(memory).not.toContain("\u001b[");
    todo = readFileSync(join(root, "docs", "todo.txt"), "utf8");
...
`,
    ...
    expect(readFileSync(join(root, "docs", "todo.txt"), "utf8")).toContain(
...
`,
    ...
    expect(readFileSync(join(root, "docs", "session-memory.log"), "utf8")).toContain(
...
writeFileSync(join(process.cwd(), "FEEDBACK.md"), "# Feedback\\n\\n---\\n");
...
`,
    ...
    const todo = readFileSync(join(root, "docs", "todo.txt"), "utf8");
    expect(todo).toContain("x 2026-07-31 (A) 2026-07-30 First task");
    expect(todo).toContain("x 2026-07-31 (B) 2026-07-30 Second task");
...
`,
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "docs/todo.txt did not change; stopping to avoid repeating the same task",
    );

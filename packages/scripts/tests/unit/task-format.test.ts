...
/*
 * `docs/todo.txt` is the plan and the session log in one file, and it is the one
 ...
 * docs/way-of-working/task-format.md, and it runs in `npm test` — which every
...
const TASK_LISTS = ["docs/todo.txt", "docs/todo-archive.txt"];
...
describe("the open plan", () => {
  it("still has the checkpoints that make /session stop", () => {
    const contents = readFileSync(resolve(process.cwd(), "docs/todo.txt"), "utf8");

...
pub fn create_worktree_for(
    authority: &(impl WorktreeAuthority + ?Sized),
    ...
    let project_root = match authority.worktree_project_root(&request.project_id) {
        Ok(root) => root,
        Err(_) => {
            return refused(
                WorktreeRefusalKind::UnknownProject,
    ...
    match authority.approve_created_worktree(&request.project_id, &destination) {

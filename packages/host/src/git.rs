...
pub use types::{
    GitAvailability, GitChangeEntry, GitChangeState, GitCommit, GitCompareRequest, GitComparison,
    GitComparisonEntry, GitDelta, GitDiff, GitDiffBuffer, GitDiffRequest, GitDiffSource,
    GitHistoryPage, GitHistoryRequest, GitScope, GitStatusSnapshot,
};

pub trait GitAuthority {
...
fn resolve_scope<A: GitAuthority + ?Sized>(
    state: &A,
    scope: &GitScope,
) -> Result<RepositoryScope, Failure> {
    let root = state
        .git_root(&scope.project_id, &scope.worktree_id)
        .map_err(|_| Failure {
            availability: GitAvailability::Denied,
...
pub fn status_for<A: GitAuthority + ?Sized>(state: &A, scope: GitScope) -> GitStatusSnapshot {
...
pub fn history_for<A: GitAuthority + ?Sized>(
    state: &A,
...
pub fn compare_for<A: GitAuthority + ?Sized>(
    state: &A,

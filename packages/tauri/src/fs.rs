...
mod tests {
    use super::{
        atomic_write, bounded_file_read, is_web_url, mutate_file_tree_in, stamp_of,
        temporary_beside, workspace_files_in, BoundedFileRead, FileTreeCreationKind,
        FileTreeMutationRequest, FileTreeMutationResult,
    };
    use std::path::{Path, PathBuf};
    ...
    fn file_tree_mutations_create_rename_and_recoverably_remove_scoped_entries() {
        ...
        let renamed = mutate_file_tree_in(
            &scratch.0,
            FileTreeMutationRequest::Rename {
                project_id: project_id.clone(),
                worktree_id: worktree_id.clone(),
    ...
    #[test]
    fn file_tree_mutations_refuse_escape_collisions_and_repository_metadata() {
        ...
        for relative_path in ["../outside.md", ".git/config", "docs/existing.md"] {
            let result = mutate_file_tree_in(
                &scratch.0,
                FileTreeMutationRequest::Create {
                    project_id: project_id.clone(),
                    worktree_id: worktree_id.clone(),

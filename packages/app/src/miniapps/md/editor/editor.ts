...
import { listIndentation } from "./lists";
...
export interface Editor {
   ...
   * caret as a position in the text, which is what §7.10's status strip and every
   * question about navigation are actually about. `head` is the moving end.
   */
   ...
   * suite's one registry — the editor performs the command and the registry
   ...
   * suite registry. Returns the state it landed in, so a caller wiring a command
   ...
   * are: §7.1 gives every chord a reader would look up to the suite's one registry,
...
export interface EditorOptions {
   ...
   *
   * §6.3 wants unsaved state visible "without adding chrome", and DESIGN.md §7.4
   * forbids a persistent dirty indicator in the document itself. So the editor
   * reports the crossing and says nothing about how it should look; the surface
   * that shows it is the caller's, and today there is not one — §7.10's status
   * strip and §6.2's dirty-close prompt are where it will land.
   */
 ...
 * project are CSS reading suite tokens — never values compiled into a script.
 *
...
export function createEditor(
   ...
   * is type, and type in this project is a stylesheet reading suite tokens.
  ...
  const view = new EditorView({
    ...
    state: EditorState.create({
      ...
      extensions: [
         ...
         * and the status strip's command — is now a suite command registered by
        ...
        // caller's, because §7.6 makes it a suite preference rather than this
        // document's opinion.
  ...
  return {
    ...
    save: () => {
       ...
       * full disk, a read-only file, a deleted directory. The strip said saved,
       * `cmd+i` agreed, and quitting lost the work — filed as audit H1, and the
       * one failure §6.3 exists to prevent.
       *

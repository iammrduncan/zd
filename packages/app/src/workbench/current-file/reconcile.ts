...
/*
 * `FileStamp` lives in platform.ts, not here: native file facts belong to the platform boundary.
 *
 * It describes what the `file_stamp` command returns, so the platform is its
 * honest home. The current-file owner consumes it through the platform facade, so the bottom
 * layer never depends on a frontend lifecycle module.
 *

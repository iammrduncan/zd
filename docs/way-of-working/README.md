...
use what you built**. What you hit while using it goes into `FEEDBACK.md` as a raw line, and the
...
`FEEDBACK.md`, and start the next loop.
...
- **The work is committed and quiet.** The gap lands where the tree is clean and nothing is
  ...
  started. That is the safe moment to interrupt, `git log`, or append to `FEEDBACK.md` without
...
`docs/session-memory.log`, waits only when another session is needed, and checks both
`FEEDBACK.md` and the task list before every invocation. Pending feedback forces a session even
...
`docs/session-memory.log` like the work-session handoffs. Preview the current task snapshot,
...
format each event without changing the raw final handoff saved to `docs/session-memory.log`.
...
- `x` or `Ctrl+C` kills the active Codex process immediately and exits with status 130. It does not
  run a recap.
- On the summary screen, `c` rechecks `FEEDBACK.md` and the live task band and continues when work
...
It appends every line beginning with `x ` to `docs/todo-archive.txt` in file order, then removes
exactly those lines from `docs/todo.txt` while preserving open work and checkpoints.
...
| `FEEDBACK.md` | **You only** | Raw notes from using the thing. No format. `!` means blocking. |
| `docs/agent-findings.md` | **The agent only** | What it hit mid-session that was out of scope, and questions the spec does not answer. |
| `docs/feedback-archive.md` | The commands | Your raw lines, verbatim, after they were triaged. |
| `CLAUDE.md` / `AGENTS.md` | You | The standing instructions, loaded into every session. |

Two inboxes, kept strictly apart. The agent never writes to `FEEDBACK.md`, and both feed
...
3. Copy [`templates/`](templates/) into place: `CLAUDE.md`, `AGENTS.md` and `FEEDBACK.md` at the
...
| `{VISION}` | Path to the vision | `docs/vision.md` |
| `{TODO}` | Path to the task list | `docs/todo.txt` |
| `{TODO_ARCHIVE}` | Where `/archive` moves finished lines. Beside `{TODO}` | `docs/todo-archive.txt` |
| `{INBOX}` | Path to the human inbox | `FEEDBACK.md` |
| `{FINDINGS}` | Path to the agent's queue | `docs/agent-findings.md` |
| `{ARCHIVE}` | Path to the raw-notes archive | `docs/feedback-archive.md` |
| `{RULES}` | The document the operating rules live in | `docs/agent-development-time-postmortem.md` |
| `{PRIOR_FINDINGS}` | What `ref:` tags point into. Drop the tag if there was no previous attempt | `docs/goals/initial-prototype/feedback.md` |
...
>   this app is not in Rust any more — see `docs/path-forward.md`.
...
to one `FEEDBACK.md` is fine. Two agents running `/session` at once is not — they will both triage
...
| `docs/vision.md` | ~300 lines, twelve numbered sections, an out-of-scope list and a terminal condition |
| `DESIGN.md` | The design system, `Status: canonical and binding`, thirteen sections |
| `docs/path-forward.md` | Phases, sessions, the stack decision, and the operating rules with their evidence |
| `docs/todo.txt` | ~120 tasks, the plan and the session log in one file |
| `docs/feedback-archive.md` | What real use produced on day one, in the user's own words |
| `docs/agent-development-time-postmortem.md` | The 18-hour autopsy every rule above is an answer to |

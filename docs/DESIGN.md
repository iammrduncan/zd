...
The application draws no replacement titlebar, branded header, persistent status bar, footer,
...
One process owns one root workbench window. The content plane extends behind the native titlebar
where supported. The application paints no title text or substitute caption.
